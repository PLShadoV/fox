...
function parseRange(req: NextRequest): { fromISO: string; toISO: string } {
  ...
  let M = Number(nowParts.find(p => p.type === 'month')!.value)
  let D = Number(nowParts.find(p => p.type === 'day')!.value)
  if (range !== 'today') {
...
export async function GET(req: NextRequest) {
  ...
  const [foxRes, rceRes] = await Promise.allSettled([
    withTimeout(foxP, FOX_TIMEOUT_MS, 'FoxESS'),
    withTimeout(rceP, RCE_TIMEOUT_MS, 'RCE'),
  ])
