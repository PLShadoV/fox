...
export async function GET(req: NextRequest){
  ...
  for(let i=1;i<lines.length;i++){ const cols=lines[i].split(sep).map(s=>s.trim()); const time=cols[0]; const priceStr=cols[1]; if(!time||!priceStr) continue; const [hh]=time.split(':').map(Number); if(!Number.isFinite(hh)) continue; const ts=new Date(`${day}T${String(hh).padStart(2,'0')}:00:00Z`).toISOString(); const price=parseFloat(priceStr.replace(',','.')); if(!Number.isNaN(price)) out.push({ timestamp: ts, price_pln_per_kwh: price }) }
